defmodule Orange.Component.Table do
  ...
  ## Attributes
    ...
    * `:current_page` - The current page number (1-based). This attribute is optional. Defaults to 1.
      ...
      case event do
        ...
        %Orange.Terminal.KeyEvent{code: {:char, ">"}} ->
          with(
            ...
            current_page = Keyword.get(attrs, :current_page, 1),
            true <- current_page < total_pages
          ) do
        ...
        %Orange.Terminal.KeyEvent{code: {:char, "<"}} ->
          ...
          if attrs[:on_page_change] && current_page > 1,
  ...
  defp rows(all_rows, column_widths, state, attrs) do
    ...
    {children, footer} =
      if state.page_size do
        current_row =
          (current_page - 1) * state.page_size + attrs[:selected_row_index] + 1
        ...
        start_index = (current_page - 1) * state.page_size
